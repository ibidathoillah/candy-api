...
class CreateLinesRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        foreach ($this->variants ?? [] as $i => $v) {
            ...
            if ($variant) {
                $rules["variants.{$i}.quantity"] = 'required|numeric|min:1|min_quantity:'.$variant->min_qty.'|max_quantity:'.$variant->max_qty.'|min_batch:'.$variant->min_batch.'|in_stock:'.$v['id'] ?? '0';
            }
    ...
    public function messages()
    {
        return [
            ...
            'variants.*.quantity.max_quantity' => trans('getcandy::validation.max_qty'),
