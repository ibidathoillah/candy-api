...
class OrderController extends BaseController
{
    ...
    public function process(
        ...
        try {
            ...
            if($request->meta["payment_code"]){
                try{
                    $client = new \GuzzleHttp\Client();
                    $response = $client->post(env('TREASURY_API_URL', 'localhost') . '/antigrvty/jewelry/payment',array(
                            'form_params'=> [
                                'amount' =>  ($order->order_total)/100,
                                'payment_code' =>  $request->meta["payment_code"],
                                'cashtag'=> $request->meta["payment_cashtag"],
                                'phone' => $request->meta["payment_phone"],
                                'pin' => $request->meta["payment_pin"],
                            ],
            ...
            if (! $order->placed_at && $request->meta["payment_code"]) {
